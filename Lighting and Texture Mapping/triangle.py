...
void main()
{
    ...
    // 计算三角形法线
    vec3 p1 = vec3(model * vec4(aPos, 1.0));
    vec3 p2 = vec3(model * vec4(aPos + vec3(0.1, 0.0, 0.0), 1.0));
    vec3 p3 = vec3(model * vec4(aPos + vec3(0.0, 0.1, 0.0), 1.0));
    
    TriangleNormal = normalize(cross(p2 - p1, p3 - p1));
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(800, 600, "Triangle Sampling", None, None)
    if not window:
        glfw.terminate()
        return
    ...
    objectColor = np.array([1.0, 0.5, 0.30], dtype=np.float32)
    ...
    while not glfw.window_should_close(window):
        ...
        glUniformMatrix4fv(glGetUniformLocation(shader, "model"), 1, GL_FALSE, model.flatten())
        glUniformMatrix4fv(glGetUniformLocation(shader, "view"), 1, GL_FALSE, view.flatten())
        glUniformMatrix4fv(glGetUniformLocation(shader, "projection"), 1, GL_FALSE, projection.flatten())
