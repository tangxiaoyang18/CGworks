...
void main()
{
    ...
    Density = clamp(1.0 / distance, 0.1, 1.0);  // 距离越远，密度越低
...
void main()
{
    ...
    // 漫反射
    float diff = max(dot(norm, lightDir), 0.0) * Density;
    ...
    float spec = pow(max(dot(viewDir, reflectDir), 0.0), 32) * Density;
...
def main():
    ...
    window = glfw.create_window(800, 600, "Vertex Sampling", None, None)
    if not window:
        glfw.terminate()
        return
    ...
    global VAO, VBO, EBO, vertices, indices
    ...
    while not glfw.window_should_close(window):
        ...
        # 设置uniform变量
        model = Matrix44.identity(dtype=np.float32)
